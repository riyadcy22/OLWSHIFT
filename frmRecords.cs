...
namespace OLWSHIFT
{
    ...
    public void LoadInventory()
        ...
        public void LoadLowSalesReport()
        {
            int i = 0; // Row counter for DataGridView
            dataGridView7.Rows.Clear(); // Clear existing rows to avoid duplication

            try
            {
                cn.Open(); // Open the SQL connection

                // SQL query to fetch low-sales data with filters for date and sales criteria
                string query = @"
            SELECT 
                p.pcode, 
                p.pdesc AS Description,
                p.cid AS CategoryID,  -- Keep CategoryID instead of CategoryName
                p.price AS Price,
                ISNULL(SUM(c.total), 0) AS TotalSales,
                ISNULL(COUNT(c.id), 0) AS SalesCount,
                ISNULL(MAX(c.sdate), NULL) AS LastSoldDate,
                p.qty AS StockLevel,
                p.reorder AS ReorderLevel
            FROM dbo.tblProduct p
            LEFT JOIN dbo.tblCart c 
                ON p.pcode = c.pcode AND c.status = 'Sold'
            WHERE 
                (c.sdate IS NULL OR CAST(c.sdate AS DATE) BETWEEN @FromDate AND @ToDate)
            GROUP BY 
                p.pcode, 
                p.pdesc, 
                p.cid,  -- Group by CategoryID
                p.price, 
                p.qty, 
                p.reorder
            HAVING 
                ISNULL(SUM(c.total), 0) < 100 OR ISNULL(COUNT(c.id), 0) < 5; -- Low sales criteria";

                // Prepare the SQL command with parameters for the date range
                cm = new SqlCommand(query, cn);
                cm.Parameters.AddWithValue("@FromDate", dateTimePicker10.Value.Date); // Start date
                cm.Parameters.AddWithValue("@ToDate", dateTimePicker9.Value.Date);   // End date

                dr = cm.ExecuteReader(); // Execute the query and fetch the data

                // Populate DataGridView with query results
                while (dr.Read())
                {
                    int stockLevel = Convert.ToInt32(dr["StockLevel"]);

                    // Only add rows with stock level > 0
                    if (stockLevel > 0)
                    {
                        i++;
                        dataGridView7.Rows.Add(
                            ...
                            dr["Description"].ToString(), // Product description
                            dr["CategoryID"].ToString(), // Category ID
                            dr["Price"].ToString(), // Product price (without dollar sign)
                            dr["TotalSales"].ToString(), // Total sales (without dollar sign)
                            dr["SalesCount"].ToString(), // Number of sales
                            dr["LastSoldDate"] == DBNull.Value ? "Never Sold" : DateTime.Parse(dr["LastSoldDate"].ToString()).ToShortDateString(), // Last sold date or "Never Sold"
                            dr["StockLevel"].ToString(), // Stock level
                            dr["ReorderLevel"].ToString() // Reorder level
                        );
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                MessageBox.Show("Error: " + ex.Message, "Load Low Sales Report", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            finally
            {
                // Close the data reader and SQL connection
        ...
        // LinkLabel click event handler to load the filtered data
        private void linkLabel11_LinkClicked_1(object sender, LinkLabelLinkClickedEventArgs e)
        {
            LoadLowSalesReport(); // Call the LoadLowSalesReport method when the link is clicked
